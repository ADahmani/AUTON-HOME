...
$SQL = "SELECT sum(`lavelinge`) as lavelinge, sum(`lavevess`) as lavevess ,sum(`chauffeau`) as chauffeau ,sum(`chauffage`) as chauffage ,sum(`four`) as four ,sum(`sechelinge`) as sechelinge ,sum(`ond1`) as ond1 ,sum(`ond2`) as ond2,sum(`chargeGW`) as chargeGW ,sum(`chargeBS`) as chargeBS FROM (SELECT * FROM `puissances` WHERE timestamp >= NOW() - INTERVAL 1 WEEK ORDER BY timestamp DESC) t1";

$result = mysql_query($SQL);
$row = mysql_fetch_assoc($result); 
//$kwj = round(($row['lavelinge'] / 1220 )/1000,2);
$kw->lavelinge['kws'] =  round(($row['lavelinge'] / 60 )/1000,2);
$kw->lavevess['kws'] =  round(($row['lavevess'] / 60 )/1000,2);
$kw->chauffeau['kws'] =  round(($row['chauffeau'] / 60 )/1000,2);
$kw->chauffage['kws']=  round(($row['chauffage'] / 60 )/1000,2);
$kw->four['kws']=  round(($row['four'] / 60 )/1000,2);
$kw->sechelinge['kws']=  round(($row['sechelinge'] / 60 )/1000,2);
$kw->ond1['kws']=  round(($row['ond1'] / 60 )/1000,2);
$kw->ond2['kws']=  round(($row['ond2'] / 60 )/1000,2);
$kw->chargeGW['kws']=  round(($row['chargeGW'] / 60 )/1000,2);
$kw->chargeBS['kws']=  round(($row['chargeBS'] / 60 )/1000,2);
...
$SQL = "SELECT sum(`lavelinge`) as lavelinge, sum(`lavevess`) as lavevess ,sum(`chauffeau`) as chauffeau ,sum(`chauffage`) as chauffage ,sum(`four`) as four ,sum(`sechelinge`) as sechelinge ,sum(`ond1`) as ond1 ,sum(`ond2`) as ond2,sum(`chargeGW`) as chargeGW ,sum(`chargeBS`) as chargeBS FROM (SELECT * FROM `puissances` WHERE timestamp >= NOW() - INTERVAL 1 MONTH ORDER BY timestamp DESC) t1";
...
$SQL = "SELECT sum(`lavelinge`) as lavelinge, sum(`lavevess`) as lavevess ,sum(`chauffeau`) as chauffeau ,sum(`chauffage`) as chauffage ,sum(`four`) as four ,sum(`sechelinge`) as sechelinge ,sum(`ond1`) as ond1 ,sum(`ond2`) as ond2,sum(`chargeGW`) as chargeGW ,sum(`chargeBS`) as chargeBS FROM (SELECT * FROM `puissances` WHERE YEAR(timestamp) = (YEAR(CURDATE())-1) ORDER BY timestamp DESC) t1";

$result = mysql_query($SQL);
$row = mysql_fetch_assoc($result); 
//$kwj = round(($row['lavelinge'] / 1220 )/1000,2);
$kw->lavelinge['kwa1'] =  round(($row['lavelinge'] / 60 )/1000,2);
$kw->lavevess['kwa1'] =  round(($row['lavevess'] / 60 )/1000,2);
$kw->chauffeau['kwa1'] =  round(($row['chauffeau'] / 60 )/1000,2);
$kw->chauffage['kwa1']=  round(($row['chauffage'] / 60 )/1000,2);
$kw->four['kwa1']=  round(($row['four'] / 60 )/1000,2);
$kw->sechelinge['kwa1']=  round(($row['sechelinge'] / 60 )/1000,2);
$kw->ond1['kwa1']=  round(($row['ond1'] / 60 )/1000,2);
$kw->ond2['kwa1']=  round(($row['ond2'] / 60 )/1000,2);
$kw->chargeGW['kwa1']=  round(($row['chargeGW'] / 60 )/1000,2);
$kw->chargeBS['kwa1']=  round(($row['chargeBS'] / 60 )/1000,2);

$SQL = "SELECT sum(`lavelinge`) as lavelinge, sum(`lavevess`) as lavevess ,sum(`chauffeau`) as chauffeau ,sum(`chauffage`) as chauffage ,sum(`four`) as four ,sum(`sechelinge`) as sechelinge ,sum(`ond1`) as ond1 ,sum(`ond2`) as ond2,sum(`chargeGW`) as chargeGW ,sum(`chargeBS`) as chargeBS FROM (SELECT * FROM `puissances` WHERE YEAR(timestamp) = (YEAR(CURDATE())-2) ORDER BY timestamp DESC) t1";

$result = mysql_query($SQL);
$row = mysql_fetch_assoc($result); 
//$kwj = round(($row['lavelinge'] / 1220 )/1000,2);
$kw->lavelinge['kwa2'] =  round(($row['lavelinge'] / 60 )/1000,2);
$kw->lavevess['kwa2'] =  round(($row['lavevess'] / 60 )/1000,2);
$kw->chauffeau['kwa2'] =  round(($row['chauffeau'] / 60 )/1000,2);
$kw->chauffage['kwa2']=  round(($row['chauffage'] / 60 )/1000,2);
$kw->four['kwa2']=  round(($row['four'] / 60 )/1000,2);
$kw->sechelinge['kwa2']=  round(($row['sechelinge'] / 60 )/1000,2);
$kw->ond1['kwa2']=  round(($row['ond1'] / 60 )/1000,2);
$kw->ond2['kwa2']=  round(($row['ond2'] / 60 )/1000,2);
$kw->chargeGW['kwa2']=  round(($row['chargeGW'] / 60 )/1000,2);
$kw->chargeBS['kwa2']=  round(($row['chargeBS'] / 60 )/1000,2);


// $SQL = "SELECT sum(`lavelinge`) as lavelinge, sum(`lavevess`) as lavevess ,sum(`chauffeau`) as chauffeau ,sum(`chauffage`) as chauffage ,sum(`four`) as four ,sum(`sechelinge`) as sechelinge ,sum(`ond1`) as ond1 ,sum(`ond2`) as ond2,sum(`chargeGW`) as chargeGW ,sum(`chargeBS`) as chargeBS FROM (SELECT * FROM `puissances` WHERE YEAR(timestamp) = (YEAR(CURDATE())-3) ORDER BY timestamp DESC) t1";

// $result = mysql_query($SQL);
// $row = mysql_fetch_assoc($result); 
// //$kwj = round(($row['lavelinge'] / 1220 )/1000,2);
// $kw->lavelinge['kwa3'] =  round(($row['lavelinge'] / 60 )/1000,2);
// $kw->lavevess['kwa3'] =  round(($row['lavevess'] / 60 )/1000,2);
// $kw->chauffeau['kwa3'] =  round(($row['chauffeau'] / 60 )/1000,2);
// $kw->chauffage['kwa3']=  round(($row['chauffage'] / 60 )/1000,2);
// $kw->four['kwa3']=  round(($row['four'] / 60 )/1000,2);
// $kw->sechelinge['kwa3']=  round(($row['sechelinge'] / 60 )/1000,2);
// $kw->ond1['kwa3']=  round(($row['ond1'] / 60 )/1000,2);
// $kw->ond2['kwa3']=  round(($row['ond2'] / 60 )/1000,2);
// $kw->chargeGW['kwa3']=  round(($row['chargeGW'] / 60 )/1000,2);
// $kw->chargeBS['kwa3']=  round(($row['chargeBS'] / 60 )/1000,2);
